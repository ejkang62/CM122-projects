...
def insertion(ref_string, consensus_string, start_index):
    ins = 0
    return ins
...
if __name__ == "__main__":
    ...
    reads_fn =  join(input_folder,'{}_aligned_1.txt'.format(input_folder))

    snps, lines = generate_consensus(reads_fn)
    output_fn = join(input_folder, 'snps_{}.txt'.format(input_folder))
    zip_fn = join(input_folder, 'snps_{}.zip'.format(input_folder))
    with open(output_fn, 'w') as output_file:
        header = '>{}\n>{}\n'.format(input_folder, 'SNP')
    ...
    output_fn2 = join(input_folder, 'consensus_{}.txt'.format(input_folder))
